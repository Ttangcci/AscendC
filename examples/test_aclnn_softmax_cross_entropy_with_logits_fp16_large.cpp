...
int main() {
  ...
  // host端先用float生成数据和golden值，再转换成aclFloat16存进设备
  ...
  std::vector<aclFloat16> featuresHostData(batch * numClasses);
  std::vector<aclFloat16> labelsHostData(batch * numClasses);
  for (int64_t k = 0; k < batch * numClasses; k++) {
    featuresHostData[k] = aclFloatToFloat16(featuresHostFloat[k]);
    labelsHostData[k] = aclFloatToFloat16(labelsHostFloat[k]);
  }
  ...
  // float16精度有限，阈值放宽一些
  if (maxLossErr < 5e-2f && maxBackpropErr < 5e-2f) {
    LOG_PRINT("\n*** TEST PASSED ***\n");
  } else {
    LOG_PRINT("\n*** TEST FAILED ***\n");
  }
